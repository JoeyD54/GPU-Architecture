	...
	void ConvertChickenBotModels()
	{
		...
		for(size_t jointIndex = 0; jointIndex < gltfModel.skins[0].joints.size(); jointIndex++)
		{
			...
			GLTF::SetVBO_BONE(gltfModel,
				...
				runModel.vbo_norm,
				(char *)&Bone0.norms[0],
				Bone0.norms.size() * sizeof(fVect3),
				Bone0.norms.size());
			...
			GLTF::SetVBO_BONE(gltfModel,
				...
				Bone0.text_coord.size() * sizeof(fVect2),
			...
			GLTF::SetVBO_BONE_index(gltfModel,
				...
				runModel.vbo_index,
				(char *)&Bone0.index[0],
				Bone0.index.size() * sizeof(iuVect3),
				Bone0.index.size() * 3);
