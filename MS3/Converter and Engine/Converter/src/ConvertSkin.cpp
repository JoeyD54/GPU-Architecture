...
namespace Azul
{
	void ConvertSkin(const char *const pName, const char *const pTargetName)
	{
		...
		{
			Skin0.joints.push_back(iuVect4((unsigned int)pJointVect4[i].x,
		...
		{
			Skin0.index.push_back(iuVect3(pisVect3[i].x, 
											pisVect3[i].y, 
											pisVect3[i].z));
		}
		...
		GLTF::SetCustomVBO(ATTRIB_LOCATION_NORM,
			...
			runModel.vbo_norm,
			(char *)&Skin0.norms[0],
			Skin0.norms.size() * sizeof(fVect3),
			Skin0.norms.size(),
			vboData::VBO_COMPONENT::FLOAT,
			vboData::VBO_TYPE::VEC3,
			vboData::VBO_TARGET::ARRAY_BUFFER
		...
		GLTF::SetCustomVBO(ATTRIB_LOCATION_TEXT,
			...
			Skin0.text_coord.size() * sizeof(fVect2),
		...
		// ------------------------------------------------
		// GLTF::SetCustomVBO(ATTRIB_LOCATION_COLOR,
		//	                 "COLOR",
		//	                 runModel.vbo_color,
		//	                 (char *)&Skin0.color[0],
		//	                 Skin0.color.size() * sizeof(fVect4),
		//	                 Skin0.color.size(),
		//	                 vboData::VBO_COMPONENT::FLOAT,
		...
		GLTF::SetCustomVBO_index(ATTRIB_LOCATION_INDEX,
			...
			runModel.vbo_index,
			(char *)&Skin0.index[0],
			Skin0.index.size() * sizeof(iuVect3),
			Skin0.index.size(),
		...
		GLTF::SetCustomVBO(ATTRIB_LOCATION_WEIGHT,
			"WEIGHTS_0",
			runModel.vbo_weights,
			(char *)&Skin0.weights[0],
			Skin0.weights.size() * sizeof(fVect4),
			Skin0.weights.size(),
			vboData::VBO_COMPONENT::FLOAT,
			vboData::VBO_TYPE::VEC4,
			vboData::VBO_TARGET::ARRAY_BUFFER
		...
		GLTF::SetCustomVBO(ATTRIB_LOCATION_JOINT,
			"JOINTS_0",
			runModel.vbo_joints,
			(char *)&Skin0.joints[0],
			Skin0.joints.size() * sizeof(iuVect4),
			Skin0.joints.size(),
			vboData::VBO_COMPONENT::UNSIGNED_INT,
			vboData::VBO_TYPE::VEC4,
			vboData::VBO_TARGET::ARRAY_BUFFER
		...
		GLTF::SetCustomVBO(ATTRIB_LOCATION_INVERSE_BIND,
			"INVERSE_BIND_0",
			runModel.vbo_invBind,
